...
const uploadPDF = multer({
  dest: path.join(__dirname, '../temp'),
  limits: {
    fileSize: 20 * 1024 * 1024, // 20MB máximo tamaño de archivo
  },
  fileFilter: (req, file, cb) => {
...
}).single('pdf'); // 'pdf' es el nombre del campo en el formulario
...
const PrestadorController = {
  ...
  async getCategoriasVirtuales(req, res) {
    handleResponse(
      res,
      () =>
        PrestadorService.getCategoriasVirtuales(
          req.params.idPlan,
          req.params.edit || false,
        ),
      Entity.CATEGORIAS
  ...
  async getNombresPrestadores(req, res) {
    handleResponse(
      res,
      () =>
        PrestadorService.getNombresPrestadores(),
      Entity.PRESTADORES
    );
  },
  ...
  async getEspecialidadesVirtuales(req, res) {
    handleResponse(
      res,
      () =>
        PrestadorService.getEspecialidadesVirtuales(
          req.params.idPlan,
          req.params.idCategoria,
          req.params.edit || false,
        ),
  ...
  async getPrestadoresVirtuales(req, res) {
    handleResponse(
      res,
      () =>
        PrestadorService.getPrestadoresVirtuales(
          req.params.idPlan,
          req.params.idCategoria,
          req.params.idEspecialidad,
          req.params.edit || false,
  ...
  /**
   * Actualiza la portada PDF para un plan y provincia específicos
   * @async
   * @param {Object} req - Objeto de solicitud Express
   * @param {Object} res - Objeto de respuesta Express
   */
  async updatePortadaPDF(req, res) {
    ...
    const handleUpload = () => new Promise((resolve, reject) => {
      uploadPDF(req, res, async (err) => {
        ...
        if (!req.file) {
          return reject(new Error('No se ha subido ningún archivo PDF'));
        }
        ...
        try {
          ...
          const result = await PrestadorService.updatePortadaPDF(
            parseInt(idPlan),
